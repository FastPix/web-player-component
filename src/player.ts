...
import { handleTitleContainer } from "./utils/DomVisibilityManager";
import { hideDefaultSubtitles } from "./utils/SubtitleHandler";
...
import { setStreamUrl, updateTimeDisplay, videoEvents } from "./utils/index";
...
class FastPixPlayer extends windowObject.HTMLElement {
  ...
  playbackRateDiv?: HTMLDivElement | any;
  ...
  forwardSeekOffset?: number | any;
  backwardSeekOffset?: number;
  parentVolumeDiv: HTMLDivElement;
  volumeButton: HTMLButtonElement;
  volumeiOSButton: HTMLButtonElement;
  volumeControl: HTMLInputElement | any;
  primaryColor?: string | any;
  loader: any;
  bottomCenterDiv?: HTMLDivElement | any;
  spacer: HTMLDivElement;
  playbackRateButton?: HTMLButtonElement | any;
  titleElement?: HTMLDivElement | any;
  debugAttribute?: string | any;
  ...
  parentLiveTitleContainer?: HTMLDivElement | any;
  liveStreamDisplay?: HTMLDivElement | any;
  ...
  constructor() {
    ...
    fastForwardButtonClickHandler(this);
    ...
    this.volumeiOSButton.style.display = "flex";
    ...
    if (this.isFirefox) {
      this.pipButton.classList.add("pip-firefox");
    } else {
      this.pipButton.classList.add("not-firefox-pip");
    }
  ...
  connectedCallback() {
    this.playbackId = this.getAttribute("playback-id");
    const customDomain = this.getAttribute("custom-domain");
    this.token = this.getAttribute("token");
    this.defaultStreamType =
      this.getAttribute("default-stream-type") || "on-demand";
    this.streamType =
      this.getAttribute("stream-type") || this.defaultStreamType || "on-demand";
    this.debugAttribute = this.hasAttribute("debug");
    this.startTimeAttribute = this.hasAttribute("start-time")
      ? this.getAttribute("start-time")
      : 0;

    // thumbnail attributes
    this.thumbnailTime =
      this.getAttribute("thumbnail-time") || this.startTimeAttribute;
    this.getThumbnailAttribute = this.getAttribute("thumbnail-time");
    this.thumbnailTimeAttribute =
      parseFloat(this.getThumbnailAttribute) || parseFloat(this.thumbnailTime);
    this.posterAttribute = this.getAttribute("poster");
    this.placeholderAttribute = this.getAttribute("placeholder");
    this.thumbnailUrlAttribute = this.getAttribute("thumbnail-url");
    this.thumbnailUrlFinal = this.thumbnailUrlAttribute
      ? this.thumbnailUrlAttribute
      : `https://images.fastpix.io`;

    // playbackrates
    this.playbackRatesAttribute = this.getAttribute("playback-rates");
    this.defaultPlaybackRateAttribute = this.getAttribute(
      "default-playback-rate"
    );

    this.titleText = this.getAttribute("title");
    this.preloadAttribute = this.getAttribute("preload");
    this.crossoriginAttribute = this.getAttribute("crossorigin");

    const defaultAccent = "#5D09C7";
    const defaultPrimary = "#F5F5F5";
    const defaultSecondary = "transparent";

    this.accentColor = this.getAttribute("accent-color") || defaultAccent;
    this.primaryColor = this.getAttribute("primary-color") || defaultPrimary;
    this.secondaryColor =
      this.getAttribute("secondary-color") || defaultSecondary;
    ...
    this.defaultDuration = this.getAttribute("default-duration");
    this.disableKeyboardControls =
      this.hasAttribute("disable-keyboard-controls") &&
      this.getAttribute("disable-keyboard-controls") !== "false";
    const getHotKeys: string | any = this.getAttribute("hot-keys");
    this.hotKeys = this.hasAttribute("hot-keys") ? getHotKeys.split(" ") : [];
    this.mutedAttribute = this.hasAttribute("muted");
    this.hasAutoPlayAttribute = this.hasAttribute("auto-play");
    this.loopAttribute = this.hasAttribute("loop");
    ...
    // data
    // initializeAnalytics(this, this.video, hlsInstance, Hls);

    if (this.loopAttribute) {
...
if (!windowObject.customElements.get("fp-player")) {
  windowObject.customElements.define("fp-player", FastPixPlayer);
}
