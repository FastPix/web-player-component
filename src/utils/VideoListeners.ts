...
const videoListeners = (context: any) => {
  if (context) {
    ...
    context.video.addEventListener("loadedmetadata", () => {
      ...
      if (context.hasAutoPlayAttribute) {
    ...
    context.progressBar.addEventListener("keydown", (event: KeyboardEvent) => {
      ...
      if (event.code === "ArrowLeft" || event.code === "ArrowRight") {
        event.preventDefault(); // Prevent the default behavior of the progress bar
        const seekOffset =
          event.code === "ArrowRight"
            ? context.getAttribute("forward-seek-offset")
              ? parseInt(context.getAttribute("forward-seek-offset"))
              : 10
            : context.getAttribute("backward-seek-offset")
        ...
        if (event.code === "ArrowUp" && context.video.volume < 1) {
          context.video.volume = Math.min(1, context.video.volume + volumeStep);
          ...
          updateVolumeControlBackground(context);
          updateVolumeButtonIcon(context);
        } else if (event.code === "ArrowDown" && context.video.volume > 0) {
          context.video.volume = Math.max(0, context.video.volume - volumeStep);
